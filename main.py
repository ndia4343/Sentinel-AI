...
def make_gauge(val, vmin, vmax, label, unit):
    """Pure SVG half-circle arc gauge."""
...
cause_lbl, cause_detail = get_cause(proc_temp, engine_rpm, torque_nm, tool_wear, air_temp)
...
bar_col    = "#3db85a" if level == "nominal" else "#d4a843" if level == "warn" else "#d84040"
mode_badge = "LIVE" if (live_mode and not st.session_state.estop) else "MANUAL"
...
<div class="status-card {sc_cls}">
  ...
  <div style="text-align:right">
    ...
    log_html = "<br>".join(st.session_state.log[-6:]) if st.session_state.log else "No events."
...
with tab_sensors:
    ...
    with left:
        st.markdown('<p class="sec-label">MANUAL SENSOR OVERRIDE</p>', unsafe_allow_html=True)
        st.markdown(
        ...
        def bpc(val, vmin, vmax, w=0.65, c=0.85):
            pct = max(0.0, min(100.0, (float(val) - vmin) / (vmax - vmin) * 100))
            col = "#d84040" if pct / 100 > c else "#d4a843" if pct / 100 > w else "#3a86ff"
            return pct, col
    ...
    with right:
        st.markdown('<p class="sec-label">CAUSE ANALYSIS</p>', unsafe_allow_html=True)
...
<div style="background:#0d1a12;border:1px solid #1a3d22;border-radius:6px;
  ...
  <div style="font-size:10px;color:#5a7060">
    All 5 features within normal operating range.</div>
        ...
        st.markdown('<p class="sec-label" style="margin-top:10px">FEATURE STATUS</p>', unsafe_allow_html=True)
        st.markdown(
            '<div style="background:#111318;border:1px solid #1e2230;'
...
with tab_charts:
    ...
    if len(labels) < 2:
        ...
        st.markdown('<p class="sec-label">SENSOR HISTORY — LAST 30 READINGS</p>', unsafe_allow_html=True)
...
with tab_alerts:
    if not st.session_state.alert_hist:
        for a in [
            {"level": "ok", "title": "System Initialized",
             "meta": "Logistic regression engine loaded · Unit-07", "time": "--:--:--"},
            {"level": "ok", "title": "All Sensors Nominal",
             "meta": "5 features stable at startup · Unit-07", "time": "--:--:--"},
        ]:
            st.session_state.alert_hist.append(a)
    
    active = [a for a in st.session_state.alert_hist if a["level"] in ("crit", "warn")]
    hist   = [a for a in st.session_state.alert_hist if a["level"] == "ok"]
    
    al, ar = st.columns([1, 1])
    
    with al:
        st.markdown('<p class="sec-label">ACTIVE ALERTS</p>', unsafe_allow_html=True)
        if active:
            st.markdown("".join(alert_row_html(a) for a in active[:6]), unsafe_allow_html=True)
        else:
            st.markdown("""
<div style="background:#0d1a12;border:1px solid #1a3d22;border-radius:5px;
            padding:12px 14px;font-family:'Courier New',monospace;
            font-size:11px;color:#3db85a">No active alerts.</div>""", unsafe_allow_html=True)
    
    with ar:
        st.markdown('<p class="sec-label">ALERT HISTORY</p>', unsafe_allow_html=True)
        if hist:
            st.markdown("".join(alert_row_html(a) for a in hist[:8]), unsafe_allow_html=True)
        else:
            st.markdown("""
<div style="background:#151820;border:1px solid #1e2230;border-radius:5px;
            padding:12px 14px;font-family:'Courier New',monospace;
            font-size:11px;color:#5a6070">No history yet.</div>""", unsafe_allow_html=True)
...
<div style="background:#0d1a2a;border:1px solid #1a3a5a;
  ...
  <div style="font-size:9px;color:#5a9fd4;letter-spacing:2px;
              margin-bottom:6px">FLEET INFORMATION</div>
  <div style="font-size:11px;color:#8ab0d0;line-height:1.9">
    Connected Nodes: 25 Industrial Machines<br>
    Active ML Node: Node-25 (Unit-07)<br>
    Data Source: 1 Real Machine → Simulated Fleet
  </div>
</div>
...
# ═══════════════════════════════════════════════
if live_mode and not st.session_state.estop and not st.session_state.override:
    time.sleep(1)
    st.rerun()
